...
if %errorlevel% neq 0 (
    ...
    echo  [!] Try running: %PYTHON% -m pip install fastapi uvicorn httpx
    pause
    exit /b 1
)
...
if not exist "%INSTALL_DIR%\.env.ecfs-lite" (
    (
        echo # ECFS Lite Config — Generated by Windows Installer
        echo ECFS_RELAY_URL=%RELAY_URL%
        echo ECFS_LITE_PORT=%LITE_PORT%
        echo ECFS_LITE_KEYS=%INSTALL_DIR%\ecfs-lite-keys.json
        echo ECFS_LITE_STATE=%INSTALL_DIR%\lite-state
